...
def GaussianFilter():
    original = cv2.imread("Images/Alevino.jpg")
    #oris= cv2.resize(original,(500,500))
    image = cv2.cvtColor(original, cv2.COLOR_BGR2GRAY)
    #ims = cv2.resize(image,(500,500))
    blur = cv2.GaussianBlur(image,(5,5),0)        
    cv2.imwrite("Images/Original.png",original)
    cv2.imwrite("Images/Blured.png",blur)
    cv2.imshow("Ajustada",blur)
    cv2.imshow("Original",original)
    cv2.waitKey(0)
    return 

def Binary():
    img = cv2.imread("204.png")
    print(img)
    ret,thresh1 = cv2.threshold(img,110,255,cv2.THRESH_BINARY)
    cv2.imwrite("204as.png",thresh1)
...
def camaraWeb():
    ...
    while(True):
        ...
        Invested = cv2.flip(gray, 1)

        cv2.imshow('frame', Invested)
